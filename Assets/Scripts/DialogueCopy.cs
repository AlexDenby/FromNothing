...
public class DialogueCopy : MonoBehaviour
{
    ...
    public GameObject CreateDialogue(string text, int person)
    {
        ...
        if (person == 0)
        {
            diaBox = GetComponent<SpikeGenCopy>().CreateInstance(playerBox, new Vector3(11 - playerBox.GetComponent<SpriteRenderer>().size.x,
        ...
        if (person == 1)
        {
            ...
            if (Random.Range(0, 2) == 0)
            {
                ...
                diaBox = GetComponent<SpikeGenCopy>().CreateInstance(johnBoxL, new Vector3(john.transform.position.x - leftright,
                john.position.y + 2, -3));
            }
            else
            {
                leftright = -johnBoxR.GetComponent<SpriteRenderer>().size.x;
                diaBox = GetComponent<SpikeGenCopy>().CreateInstance(johnBoxR, new Vector3(john.transform.position.x - leftright,
                john.position.y + 3, -3));
            }

            diaBox.GetComponent<CharacterCopy>().defaultPos = new Vector3(-leftright, 0.5f);

        }

        GameObject textObj = GetComponent<SpikeGenCopy>().CreateInstance(wordTable[text], new Vector3(diaBox.transform.position.x,
